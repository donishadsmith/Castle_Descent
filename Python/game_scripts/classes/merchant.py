...
class merchant_class:
    ...
    def menu(self, player, game_sequence):
        ...
        while not player_action == "e":
            ...
            new_line(1)
            print(
                f"\U0001f4b0: {player.money} | Number in inventory: {player.hidden_item_inventory[object]}"
            )
            new_line(1)
            ...
            if player_action in ["a", "d"]:
                self.shop_inventory = cursor.move_cursor(
                    object=self.shop_inventory,
            ...
            print(
                "".join(
                    [
                        (
                            "{}".format(str(cell) + " ")
                            if cell in ["\u2770", self.selection_menu[2]]
                            else cell
                        )
                        for cell in self.selection_menu
                    ]
                )
            )
            print("___________________________________________")
